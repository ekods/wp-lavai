<?php
add_action('add_meta_boxes', 'add_collection_group_page_meta');
function add_collection_group_page_meta()
{
    global $post;
    ...
    if(!empty($post))
    {
        ...
        if($pageTemplate == 'templates/page-collection-group.php' )
        {
          add_meta_box( 'repeatable-fields', 'Collection Group Item', 'repeatable_meta_box_collection_group_page_item_display', 'page', 'normal', 'default');
...
function repeatable_meta_box_collection_group_page_item_display() {
    global $post;
    $collection_group_page_item = get_post_meta($post->ID, 'collection_group_page_item', true);
    wp_nonce_field( 'hhs_repeatable_meta_box_nonce_collection_group_page_item', 'hhs_repeatable_meta_box_nonce_collection_group_page_item' );


    print_r(get_post_meta(get_queried_object_id(), 'collection_group_page_item', true));
    ...
    <div id="repeatable-fieldset-one" width="100%">
      ...
      if ( $collection_group_page_item ) :

        sort($collection_group_page_item);
        foreach ( $collection_group_page_item as $field ) {
      ?>
...
function repeatable_meta_box_collection_group_page_item_save($post_id) {
    if ( ! isset( $_POST['hhs_repeatable_meta_box_nonce_collection_group_page_item'] ) ||
    ! wp_verify_nonce( $_POST['hhs_repeatable_meta_box_nonce_collection_group_page_item'], 'hhs_repeatable_meta_box_nonce_collection_group_page_item' ) )
        return;
    ...
    $old = get_post_meta($post_id, 'collection_group_page_item', true);
    $new = array();
    ...
    for ( $i = 0; $i < $count; $i++ ) {
      if ( $title[$i] != '' ) :
        ...
        $new[$i]['description'] = $description[$i];

        $new[$i]['images'] = stripslashes( $images[$i] );
      endif;
    }
    if ( !empty( $new ) && $new != $old )
        update_post_meta( $post_id, 'collection_group_page_item', $new );

    elseif ( empty($new) && $old )
        delete_post_meta( $post_id, 'collection_group_page_item', $old );
}
add_action('save_post', 'repeatable_meta_box_collection_group_page_item_save');
